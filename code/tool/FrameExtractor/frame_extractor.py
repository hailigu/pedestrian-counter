        ...
        if file_ext.lower() == '.h264':
            ...
            while (True):
                ...
                if frame_indexes.count(index_count) > 0:
                    ...
                    # cv2.imwrite(os.path.join(save_path, '{0}_{1:0>6d}.png'.format(file_name, index_count)), frame,
                    #             [cv2.cv2.IMWRITE_PNG_COMPRESSION, 2])
            ...
            for frame_index in frame_indexes:
                ...
                cv2.imwrite(os.path.join(save_path, '{0}_{1:0>6d}.jpg'.format(file_name, frame_index)), frame,
                            [cv2.IMWRITE_JPEG_QUALITY, 100])
                # cv2.imwrite(os.path.join(save_path, '{0}_{1:0>6d}.png'.format(file_name, frame_index)), frame,
